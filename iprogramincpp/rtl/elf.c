...
static uintptr_t RtlpResolveSymbolAddress(PELF_DYNAMIC_INFO DynInfo, int SymbolIndex, uintptr_t LoadBase)
{
	...
	DbgPrint("Lookup of symbol '%s' is impossible, sorry", SymbolName);
	RtlAbort();
	return 0;
}
...
bool RtlParseDynamicTable(PELF_DYNAMIC_ITEM DynItem, PELF_DYNAMIC_INFO Info, uintptr_t LoadBase)
{
	...
	for (; DynItem->Tag != DYN_NULL; DynItem++)
	{
		switch (DynItem->Tag)
		{
			...
			case DYN_NEEDED:
				DbgPrint("DYN_NEEDED not supported. What the hell are you doing?");
				return false;
...
bool RtlLinkPlt(PELF_DYNAMIC_INFO DynInfo, uintptr_t LoadBase, UNUSED const char* FileName)
{
	...
	for (i = 0; i < DynInfo->PltRelocationCount; i += Increment)
	{
		...
		{
			SymbolAddress = 0;
			DbgPrint("Dude, this symbol '%s' SHOULD NOT be undefined. What are you doing?", SymbolName);
			RtlAbort();
		}
